...
switch (estado)
{
    case "patrulha":
    {
        ...
        if (estado != "hit")
        {
            if (tempo_recuperacao > 0)
            {
                tempo_recuperacao--;
                velh = 0;
                sprite_index = spr_inimigo_krolium_parado;
            }
            else
            {
                velh = 0;
                ...
                sprite_index = spr_inimigo_krolium_andando;
            }
        }
        break;
    }
    ...
    case "attack":
    {
        ...
        if (estado != "hit")
        {
            if (!atacando)
            {
                obj_inimigo_krolium.atacando = true;
                tempo_ataque = 0;
            }
            ...
            {
                dano = instance_create_layer(x + sprite_width / 5, y - sprite_height / 3, layer, obj_dano);
                dano.dano = ataque;
                dano.pai = id;
                dano.image_xscale = 4;
    ...
    case "ataque2":
    {
        ...
        if (estado != "hit")
        {
            ...
            velh = 0;
            if (sprite_index != spr_inimigo_krolium_atq2)
            {
                image_index = 0;
                posso = true;
                dano = noone;
            }
            sprite_index = spr_inimigo_krolium_atq2;
			if (!atq2_som && image_index >= 4)
			{
				if (global.sound_effects_on) 
				{
	                audio_play_sound(snd_boss_slash, 2, false, 3);
            ...
            if (image_index >= 2 && dano == noone && image_index < 6 && posso)
            {
    ...
    case "hit":
    {
        ...
        sprite_index = spr_inimigo_krolium_dano;
    ...
    case "morto":
    {
        ...
        sprite_index = spr_inimigo_krolium_morte;
		global.boss_krolium_derrotado = true;
		// Incremento dos atributos apenas uma vez
	    if (obj_player.atributos_incrementados_2 = false)
	    {
	        ...
	        obj_player.show_level_up_message_2 = true; // Exibir mensagem
	        obj_player.level_up_timer = room_speed * 3; // Mensagem por 3 segundos

	        obj_player.atributos_incrementados_2 = true; // Marca como incrementado
	    }

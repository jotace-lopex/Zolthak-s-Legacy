...
switch(estado)
{
	case"parado":
	{
		velh = 0;
		timer_estado++;
		if(sprite_index != spr_inimigo_sk_parado)
		{
			image_index = 0;
		}
		sprite_index = spr_inimigo_sk_parado;
	...
	case "walk":
	{
	    ...
	    if (sprite_index != spr_inimigo_sk_andando) {
	        image_index = 0;
	        velh = choose(1, -1); // Escolhe direção inicial: direita (1) ou esquerda (-1)
	    }
	    sprite_index = spr_inimigo_sk_andando;
	...
	{
		velh = 0;
		if (sprite_index != spr_inimigo_sk_dano)
		{
	...
	{
		velh = 0;
		if (sprite_index != spr_inimigo_sk_atq)
		{
			image_index = 0;
			posso = true;
			dano = noone;
		}
		sprite_index = spr_inimigo_sk_atq;
	...
	{
		velh = 0;
		if (sprite_index != spr_inimigo_sk_morte)
		{
			//Iniciando o que for preciso para esse estado
			image_index = 0; 
		}
		sprite_index = spr_inimigo_sk_morte;

...
switch (estado)
{
    ...
    case "attack":
    {
        ...
        if (estado != "hit")
        {
            if (!atacando)
            {
                obj_inimigo_igneez.atacando = true;
                tempo_ataque = 0;
            }
			...
			if (!atq1_som)
			{
				...
				{
					audio_play_sound(snd_boss_sword_impact, 2, false, 3);
    ...
    case "ataque2":
    {
        ...
        if (estado != "hit")
        {
			...
			if (!atq2_som && image_index >= 11)
			{
				if (global.sound_effects_on) 
				{
	                audio_play_sound(snd_boss_atk_2, 2, false, 3);
            ...
            {
                dano = instance_create_layer(x + sprite_width / -25, y - sprite_height / 5, layer, obj_dano);
                dano.dano = ataque * 2; // Causa mais dano
                dano.pai = id;
                dano.image_xscale = 5.5;
                dano.image_yscale = 2;
                posso = false;
				screenshake(20);
            }
    ...
    case "morto":
    {
		...
		global.boss_igneez_derrotado = true;
		// Incremento dos atributos apenas uma vez
	    if (obj_player.atributos_incrementados = false)
	    {
	        ...
	        obj_player.show_level_up_message = true; // Exibir mensagem
	        obj_player.level_up_timer = room_speed * 3; // Mensagem por 3 segundos

	        obj_player.atributos_incrementados = true; // Marca como incrementado
	    }
        ...
        if (image_index >= image_number - 1)
        {
			...
			if (image_alpha <= 0) 
            {
				...
				{				
	                instance_create_layer(x + sprite_width/7, y - sprite_height/2.5, layer, obj_drop_power_up_jump);
